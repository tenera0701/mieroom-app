...
MANUAL_SECTIONS = [
    ...
    {'cat': '契約管理', 'title': '契約管理（請求書・契約方法・連帯保証・メモの記入）', 'body':
     ...
     '請求書・契約方法・連帯保証・メモは顧客管理表と同じくセルを直接クリックして記入します。請求書はクリックで●/×切替、契約方法（電子/書面）と連帯保証（有/無）はクリックでプルダウン選択、メモはクリックで入力欄が開きEnterか欄外クリックで保存されます。'},
    ...
    {'cat': '来店・予約', 'title': '来店管理と予約システムのQR', 'body':
     ...
     '各種設定→予約システムではWeb予約の受付時間・枠・定休日を設定し、予約ページのQR印刷・ダウンロード・URLコピーができます。入った予約はカレンダーに自動で入ります。'},
